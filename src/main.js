...
Vue.component('v-grid-list', GridList)
Vue.component('v-tag-block', TagBlock)
...
Vue.prototype.$get = async (params) => {
...
Vue.prototype.$safeGet = async (params) => {
    try {
        const response = await axios.get("client/api", {
            params: Object.assign({
                response: "json"
...
var queryJobResult = (jobid, message, callback, callbackParam) => {
    ...
    return axios.get('/client/api', {
        ...
        if (result.queryasyncjobresultresponse.jobstatus == 1) {    //Job has successfully completed
            ...
            if (callback) {
                callback(result.queryasyncjobresultresponse)
            }
