  ...
  <div class="global-settings">
    <div class="global-settings-content">
      ...
      <div class="global-settings-table">
        <v-table :multiple-sort="false" :min-height="350" even-bg-color="#f2f2f2" row-hover-color="#eee" row-click-color="#edf7ff"
          :paging-index="(pageIndex-1)*pageSize" :loading-content="loadingContent" :is-loading="isLoading" :width="1200" :columns="columns"
          :table-data="tableData" :cell-edit-done="cellEditDone" @on-custom-comp="customCompFunc">
        </v-table>
        <div class="mt20 mb20 bold paginationDiv"></div>
        <Page :total="total" show-elevator @on-change="pageChange" :page-size="pageSize" style="text-align: center"></Page>
        </v-pagination>
      </div>
    </div>
    <Modal v-model="isShow" @on-ok="setDialogVisible" :title="isLDAP?'配置LDAP':'添加Baremetal Rack配置'">
      <div class="bodyDiv">
        <Form :model="LDAPForm" :rules="ldapRules" ref="LDAPForm" :label-width="100">
          <div class="bodyDivA" v-if="isLDAP">
            <p>
              <FormItem label="名称" prop="hostname">
                <Input id="hostname" class="inputCla" style="width: 70%" v-model="LDAPForm.hostname"></Input>
              </FormItem>
            </p>
            <p>
              <FormItem label="端口" prop="port">
                <Input id="port" class="inputCla" style="width: 70%" v-model="LDAPForm.port"></Input>
              </FormItem>
            </p>
          </div>
        </Form>
        <Form :model="BRForm" :rules="brRules" ref="BRForm" :label-width="100">
          <div class="bodyDivA" v-if="isBR">
            <p>
              <FormItem label="URL" prop="url" style="width: 70%;">
                <Input id="url" class="inputCla" style="margin-left: 60px" v-model="BRForm.url"></Input>
              </FormItem>
            </p>
          </div>
          ...
          if (r == true) {
            this.$http
              ...
              .then(
                function (response) {
                  if (
                    response.updateconfigurationresponse != null &&
                    response.updateconfigurationresponse.configuration
                  ) {
          ...
          this.$http
            ...
            .then(
              function (response) {
                ...
                this.tableData =
                  response.listconfigurationsresponse.configuration;
              }.bind(this)
            );
        } else {
          this.$http
          ...
          this.$http
            ...
            .then(
              function (response) {
                ...
                this.tableData =
                  response.listhypervisorcapabilitiesresponse.hypervisorCapabilities;
              }.bind(this)
            );
        } else {
          this.$http
          ...
          this.$http
            .get("client/api", {
              params: {
                ...
                response: "json",
                hostname: this.LDAPForm.hostname,
          ...
          this.$http
            .get("client/api", {
              params: {
                ...
                response: "json",
                baremetalrcturl: this.BRForm.url
              }
            })
            .then(
              function (response) {
                ...
                this.$http
                  ...
                  .then(
                    function (response) {
                      alert(
                        response.queryasyncjobresultresponse.jobresult.errortext
                      );
                    }.bind(this)
                  );
              }.bind(this)
